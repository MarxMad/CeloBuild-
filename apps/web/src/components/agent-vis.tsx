...
export function AgentVis() {
  ...
  useEffect(() => {
    ...
    // Positioning in a Triangle (Massive spacing for double size)
    // Top
    trendAgent.group.position.set(0, 3.5, 0)
    // Bottom Right
    eligibilityAgent.group.position.set(3.5, -2.0, 0)
    // Bottom Left
    rewardAgent.group.position.set(-3.5, -2.0, 0)

    // Double scale again (4.0)
    const scale = 4.0
    ...
    const points = []
    points.push(new THREE.Vector3(0, 3.5, 0))      // Top
    points.push(new THREE.Vector3(3.5, -2.0, 0))   // Bottom Right
    points.push(new THREE.Vector3(-3.5, -2.0, 0))  // Bottom Left
    points.push(new THREE.Vector3(0, 3.5, 0))      // Back to Top to close loop

    const lineGeometry = new THREE.BufferGeometry().setFromPoints(points)
    const lines = new THREE.Line(lineGeometry, lineMaterial)
    mainGroup.add(lines)

    camera.position.z = 14 // Pull back significantly to fit the massive formation
